...
md""" Veamos los errores que esta nos da"""
...
\vdots &\vdots &\vdots &\vdots &\vdots \\ O&O&O&R_{nn-1}&R_{nn} \end{bmatrix}$$
...
# ╔═╡ 3d1cd04b-f55e-4e25-90fc-8636d4e9f1a7
begin
function Housev(x)
    n = length(x)
    v = ones(size(x))
...
# ╔═╡ dce32ade-9df5-4ea7-8dd9-a9f715d91e75
...
# ╔═╡ 434a54db-2aac-4806-a921-db643ce3b95d
begin
...
# ╔═╡ 07adc427-c71c-40f6-ba3d-3c8aa60144e5
md""" Por ejemplo continuando con la matriz inicial se tiene lo siguiente:"""

# ╔═╡ 9a94b425-b5ff-42ad-a9cc-0bd114116623
begin
BH =rand(5,5)
...
# ╔═╡ abbf2349-0072-4db5-808a-1706730eed09
display(U₁)

# ╔═╡ 637f964c-e78f-407e-8cf6-e4c92aa6920f
...
# ╔═╡ a6375ed8-fb53-44fb-afbb-245f947c23bd
function Givens(a,b)
    if b==0
        c = 1
        s = 0
...
# ╔═╡ 773e72a5-3a0b-46ec-ab2c-94a299bc9d82
begin
...
# ╔═╡ 7c16e5e9-e8bb-4d6f-8efd-428fa24882f1
begin
H₂=RealSchur(BH)
end

# ╔═╡ f2a76515-ed3c-4a9a-bc2e-7e2e246ff206
begin 
C=rand(100,100)
H₃=RealSchur(C)
end

# ╔═╡ 37b33943-2fb4-41bf-9e2b-f9bc0e239c00
...
# ╔═╡ 50ae5eb2-f6fe-4bfd-bc64-5e10133067bc
md"""#### 3. Testeo contra Shur

En primer lugar, se evaluarán los tiempos de ejecución de ambas metodologías:"""

# ╔═╡ 7c137c74-dde9-4729-adbe-bfb7010128b5
@benchmark H=RealSchur(rand(100,100))

# ╔═╡ 435d5204-4b65-4fcc-9c1a-ce4eefd21df5
@benchmark H=schur(rand(100,100))

# ╔═╡ 67427c0a-f294-4910-8813-fe613b14818f
begin
	TRS = []
	for i in collect(2:10)
		a=0
		for j=1:30 
			a += (@timed RealSchur(rand(i , i)))[2]
		end
		append!(TRS, a/30)
	end
end

# ╔═╡ fe8932a0-8683-4b67-a95f-4c22b7c9ce84
begin
	TS = []
	for i in collect(2:10)
		a=0
		for j=1:30
			a += (@timed schur(rand(i , i)))[2]
		end
		append!(TS, a/30)
	end
end

# ╔═╡ bda82272-a85c-4a77-b63c-2818ebebd341
...
# ╔═╡ 7531bdd3-73cb-462d-bba3-e6460329950f
begin
...
# ╔═╡ 6c974398-0f6a-462e-9b2e-7ff1506d6404
begin
...
# ╔═╡ 999e51f4-0bd3-4eb1-9244-4f68db7c0512
begin
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "87b8d169e4c55fb257205c36a63b74dacd903aa7"
